...
- Added Pico All management for PIV, OpenPGP, SmartCard-HSM, Audit and firmware workflows
- Added English and Simplified Chinese, flat software settings and a searchable IANA time-zone list with daylight-saving support
- Added searchable, filterable lists with independent scrolling for keys, objects, slots and security events
- HSM objects accept text or file imports up to 1,800 bytes; new IDs are assigned automatically
- Improved PIN forms, default-value handling, validation messages, reset dialogs and elapsed-time feedback
...
- 新增 Pico All 的 PIV、OpenPGP、SmartCard-HSM、审计和固件管理
- 新增英语、简体中文、平铺式软件设置和可搜索的 IANA 时区列表，支持夏令时
- 密钥、对象、槽位和安全事件列表支持搜索、筛选和独立滚动
- HSM 对象支持文本或文件导入，大小上限为 1,800 字节，新对象 ID 自动分配
- 改进 PIN 表单、默认值处理、必填校验、重置确认和操作计时
